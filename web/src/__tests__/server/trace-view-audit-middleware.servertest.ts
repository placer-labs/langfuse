...
// Both trace lookups are mocked unconditionally: `enforceTraceAccess` reads the
// legacy traces table and falls back to the events table under
// LANGFUSE_MIGRATION_V4_WRITE_MODE=dual. Mocking both keeps the test
...
// (v3) and `events.byTraceId` (v4 events-backed). The per-row table-cell fetch
// `traces.byId` must NOT be audited. That the audited paths still resolve to
// real prod procedures is guarded separately by
// recordTraceViewAudit.drift.servertest.ts.
//
// Each path uses the SAME procedure it uses in prod: on this branch both
// routers mount protectedGetTraceProcedure.
...
describe("trace-view audit in tRPC trace-access middleware", () => {
  ...
  it("records a read audit for the events-backed detail view (events.byTraceId)", async () => {
    // The events-backed UI fetches trace content via events.byTraceId, not
    // traces.byIdWithObservationsAndScores. It hits the same middleware and
    // must be audited too, or the audit guarantee silently fails for those
    // views.
    const { caller } = createTestCaller({ session: createMemberSession() });
